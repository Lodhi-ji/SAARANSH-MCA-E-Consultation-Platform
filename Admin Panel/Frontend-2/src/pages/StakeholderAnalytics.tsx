...
const StakeholderAnalytics = () => {
  ...
  const stanceByType = allComments.reduce((acc, comment) => {
    ...
    acc[comment.stakeholderType][comment.stance] = ((acc[comment.stakeholderType][comment.stance] as number) || 0) + 1;
    return acc;
  ...
  const averageQualityByType = Object.entries(
    allComments.reduce((acc, comment) => {
      ...
      acc[comment.stakeholderType].total += (comment.confidenceScore_based_on_ensemble_model as number) || 0;
      acc[comment.stakeholderType].count += 1;
      return acc;
    }, {} as Record<string, { total: number; count: number }>)
  ).map(([type, data]) => ({
    type,
    average: ((data as any).total / (data as any).count).toFixed(1)
  }));
