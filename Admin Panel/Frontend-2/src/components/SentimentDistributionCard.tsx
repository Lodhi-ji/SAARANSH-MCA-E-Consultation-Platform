...
const SentimentDistributionCard: React.FC<SentimentDistributionCardProps> = ({ comments, avgConfidence }) => {
    ...
    const data = useMemo(() => {
        ...
        comments.forEach(c => {
            const stance = c.stance as keyof typeof counts;
            if (counts[stance] !== undefined) {
                counts[stance]++;
            }
        });
        ...
        <Card className="h-full border-slate-200 shadow-sm hover:shadow-md transition-shadow duration-200">
            ...
            <CardContent>
                ...
                <div className="mt-6 flex justify-center">
                    <Badge variant="outline" className="px-3 py-1 bg-slate-50 text-slate-600 border-slate-200 font-normal">
                        Confidence Score: <span className="font-semibold text-slate-800 ml-1">{avgConfidence.toFixed(1)} / 5.0</span>
                    </Badge>
                </div>
            </CardContent>
        </Card>
